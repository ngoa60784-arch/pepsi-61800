...
import { KaliArsenalPanel } from "./kali-arsenal-panel"
...
function isKaliArsenalEditor(name: string, command: string, args: string): boolean {
    if (name.trim() === "kali-arsenal") return true
    const argList = args.trim() ? args.trim().split(/\s+/) : []
    return command.trim() === "python3" && argList.some((a) => a.includes("ssh_mcp.py"))
}
...
export function McpPage() {
    ...
    const showKaliPanel = isKaliArsenalEditor(name, command, args)
    ...
    return (
        <>
            ...
            <Dialog open={editorOpen} onOpenChange={(open) => (!open ? resetForm() : setEditorOpen(true))}>
                <DialogContent className="max-h-[min(90vh,900px)] max-w-4xl overflow-y-auto">
                    ...
                    <form onSubmit={editing ? handleSave : handleAdd} className="space-y-4">
                        ...
                        {showKaliPanel && (
                            <KaliArsenalPanel envText={env} onEnvChange={setEnv} disabled={draftProbing} />
                        )}
                        ...
                        <div className="space-y-3">
                            ...
                            {draftProbeStatus && (
                                <div
                                    className={`rounded-md border px-3 py-2 text-xs ${draftProbeStatus.type === "error" ? "border-destructive bg-destructive/10 text-destructive" : "alert-success"}`}
                                >
                                ...
                                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 mt-3">
                                    ...
                                    {showKaliPanel && (
                                        <p className="text-xs text-muted-foreground sm:col-span-2">
                                            SSH 凭据已在上方配置；其它 MCP 环境变量可在此追加（每行 KEY=VALUE）。
                                        </p>
                                    )}
            ...
            {probeStatus && (
                <div
                    className={`rounded-md border px-4 py-2 text-sm ${probeStatus.type === "error" ? "border-destructive bg-destructive/10 text-destructive" : "alert-success"}`}
                >
                    {probeStatus.message}
                </div>
            )}
