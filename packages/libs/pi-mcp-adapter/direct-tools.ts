...
export function buildProxyDescription(
  ...
  for (const serverName of Object.keys(config.mcpServers)) {
    if (visibleServers && !visibleServers.has(serverName)) continue;
    const entry = cache?.servers?.[serverName];
    const definition = config.mcpServers[serverName];
    const toolCount = (entry?.tools?.filter((tool) => !visibleToolNames || visibleToolNames.has(formatToolName(tool.name, serverName, config.settings?.toolPrefix ?? "server"))).length) ?? 0;
...
function normalizeToolArgs(params: unknown): Record<string, unknown> {
...
export function createDirectToolExecutor(
  ...
  return async function execute(_toolCallId, params) {
    const toolArgs = normalizeToolArgs(params);
    let state = getState();
