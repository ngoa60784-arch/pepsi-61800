...
export function getToolNamePrefix(
  serverName: string,
  mode: "server" | "none" | "short"
): string {
  const serverPrefix = getServerPrefix(serverName, mode);
  return serverPrefix ? `mcp_${serverPrefix}_` : "mcp_";
}
...
export function formatToolName(
  ...
  return `${getToolNamePrefix(serverName, prefix)}${toolName}`;
}
