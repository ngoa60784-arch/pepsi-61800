...
import { openUrl, parallelLimit, resolveMcpConfigPath } from "./utils.js";
...
export async function initializeMcp(
  ...
  const configPath = resolveMcpConfigPath(configPathOverride, pi.getFlag("mcp-config") as string | undefined);
  ...
  const cachePath = getMetadataCachePath(configPath);
  const cacheFileExists = existsSync(cachePath);
  let cache = loadMetadataCache(configPath);
  let bootstrapAll = false;

  if (!cacheFileExists) {
    bootstrapAll = true;
    saveMetadataCache({ version: 1, servers: {} }, configPath);
  } else if (!cache) {
    cache = { version: 1, servers: {} };
    saveMetadataCache(cache, configPath);
  }
  ...
  if (envDirect !== "__none__") {
    ...
    const currentCache = loadMetadataCache(configPath);
    for (const [name, definition] of serverEntries) {
      ...
      const bootstrapResults = await parallelLimit(
        ...
        async (name) => {
          ...
          try {
            ...
            toolMetadata.set(name, metadata);
            updateMetadataCache(state, name, configPath);
            return { name, ok: true };
          } catch {
  ...
  lifecycle.setReconnectCallback((serverName) => {
    ...
    updateMetadataCache(state, serverName, configPath);
...
export function updateMetadataCache(state: McpExtensionState, serverName: string, configPathOverride?: string): void {
  const connection = state.manager.getConnection(serverName);
  if (!connection || connection.status !== "connected") return;
  ...
  const configPath = configPathOverride ?? state.configPath;
  const existing = loadMetadataCache(configPath);
  ...
  saveMetadataCache({ version: 1, servers: { [serverName]: entry } }, configPath);
}

export function flushMetadataCache(state: McpExtensionState, configPathOverride?: string): void {
  for (const [name, connection] of state.manager.getAllConnections()) {
    if (connection.status === "connected") {
      updateMetadataCache(state, name, configPathOverride);
    }
  }
}
