...
import { getToolNamePrefix, parseUiPromptHandoff } from "./types.js";
...
export interface McpVisibilityPolicy {
  visibleServers?: Set<string>;
  visibleToolNames?: Set<string>;
}

function isServerVisible(policy: McpVisibilityPolicy | undefined, serverName: string): boolean {
  if (!policy?.visibleServers) return true;
  return policy.visibleServers.has(serverName);
}

function isToolVisible(policy: McpVisibilityPolicy | undefined, serverName: string, toolName: string): boolean {
  if (!isServerVisible(policy, serverName)) return false;
  if (!policy?.visibleToolNames) return true;
  return policy.visibleToolNames.has(toolName);
}

function filterVisibleToolMetadata(
  metadata: ToolMetadata[] | undefined,
  serverName: string,
  policy?: McpVisibilityPolicy,
): ToolMetadata[] {
  if (!metadata) return [];
  return metadata.filter((tool) => isToolVisible(policy, serverName, tool.name));
}
...
export function executeStatus(state: McpExtensionState, policy?: McpVisibilityPolicy): ProxyToolResult {
  const servers: Array<{ name: string; status: string; toolCount: number; failedAgo: number | null }> = [];

  for (const name of Object.keys(state.config.mcpServers)) {
    if (!isServerVisible(policy, name)) continue;
    const connection = state.manager.getConnection(name);
    const metadata = filterVisibleToolMetadata(state.toolMetadata.get(name), name, policy);
...
export function executeDescribe(state: McpExtensionState, toolName: string, policy?: McpVisibilityPolicy): ProxyToolResult {
  let serverName: string | undefined;
  let toolMeta: ToolMetadata | undefined;

  for (const [server, metadata] of state.toolMetadata.entries()) {
    if (!isServerVisible(policy, server)) continue;
    const found = findToolByName(metadata, toolName);
    if (found && isToolVisible(policy, server, found.name)) {
      serverName = server;
      toolMeta = found;
      break;
    }
...
export function executeSearch(
  ...
  for (const [serverName, metadata] of state.toolMetadata.entries()) {
    if (!isServerVisible(policy, serverName)) continue;
    if (server && serverName !== server) continue;
    for (const tool of metadata) {
      if (!isToolVisible(policy, serverName, tool.name)) continue;
      if (pattern.test(tool.name) || pattern.test(tool.description)) {
        matches.push({
          server: serverName,
          tool,
...
export function executeList(state: McpExtensionState, server: string, policy?: McpVisibilityPolicy): ProxyToolResult {
  if (!state.config.mcpServers[server] || !isServerVisible(policy, server)) {
    return {
  ...
  const metadata = filterVisibleToolMetadata(state.toolMetadata.get(server), server, policy);
...
export async function executeConnect(state: McpExtensionState, serverName: string, policy?: McpVisibilityPolicy): Promise<ProxyToolResult> {
  ...
  try {
    ...
    return executeList(state, serverName, policy);
  } catch (error) {
...
export async function executeCall(
  ...
  if (!serverName && !toolMeta && prefixMode !== "none") {
    const candidates = Object.keys(state.config.mcpServers)
      .map(name => ({ name, prefix: getToolNamePrefix(name, prefixMode) }))
      .filter(c => c.prefix && toolName.startsWith(c.prefix))
