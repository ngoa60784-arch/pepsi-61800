...
import type { McpVisibilityPolicy } from "./proxy-modes.js";
...
export async function showStatus(state: McpExtensionState, ctx: ExtensionContext, policy?: McpVisibilityPolicy): Promise<void> {
  if (!ctx.hasUI) return;

  const lines: string[] = ["MCP Server Status:", ""];

  for (const name of Object.keys(state.config.mcpServers)) {
    if (policy?.visibleServers && !policy.visibleServers.has(name)) continue;
    const connection = state.manager.getConnection(name);
    const metadata = (state.toolMetadata.get(name) ?? []).filter((tool) => !policy?.visibleToolNames || policy.visibleToolNames.has(tool.name));
...
export async function showTools(state: McpExtensionState, ctx: ExtensionContext, policy?: McpVisibilityPolicy): Promise<void> {
  if (!ctx.hasUI) return;

  const allTools = [...state.toolMetadata.entries()]
    .flatMap(([serverName, tools]) => tools.filter((tool) => (!policy?.visibleServers || policy.visibleServers.has(serverName)) && (!policy?.visibleToolNames || policy.visibleToolNames.has(tool.name))))
    .map((m) => m.name);
...
export async function openMcpPanel(
  ...
  const configPath = state.configPath ?? pi.getFlag("mcp-config") as string | undefined ?? configOverridePath;
  const cache = loadMetadataCache(configPath);
  const provenanceMap = getServerProvenance(configPath);
  ...
  const callbacks: McpPanelCallbacks = {
    ...
    refreshCacheAfterReconnect: (serverName: string) => {
      const freshCache = loadMetadataCache(configPath);
